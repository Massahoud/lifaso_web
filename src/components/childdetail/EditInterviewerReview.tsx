...
import { updateInvestigatorNote } from "../../services/childService";
...
interface ModalProps {
  ...
  onSave: (note: InvestigatorNote) => void;
}
...
const InvestigatorNoteModal: React.FC<ModalProps> = ({ isOpen, onClose, note, onSave }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateInvestigatorNote(note.id, currentNote);
      onSave(currentNote);
  ...
  return (
    <div className="fixed inset-0 bg-gray-40 bg-opacity-50 backdrop-blur flex justify-end z-50">
      <div className="bg-white w-1/3 h-full p-6 shadow-lg overflow-y-auto">
        ...
        {message && (
          <div className={`fixed bottom-5 left-1/2 transform -translate-x-1/2 px-4 py-2 text-white rounded-lg shadow-lg ${
            message.type === "success" ? "bg-green-500" : "bg-red-500"
          }`}>
            {message.type === "success" ? <CheckCircle className="inline w-5 h-5 mr-2" /> : <XCircle className="inline w-5 h-5 mr-2" />}
            {message.text}
          </div>
        )}
      </div>
